use windows::Win32::{Foundation::HWND, UI::{Accessibility::HWINEVENTHOOK, WindowsAndMessaging::{CHILDID_SELF, CW_USEDEFAULT, EVENT_SYSTEM_FOREGROUND, OBJID_WINDOW, WS_EX_LAYERED, WS_EX_NOACTIVATE, WS_EX_TOOLWINDOW, WS_EX_TOPMOST, WS_POPUP, WS_SYSMENU}}};

use crate::{data::{window::Window, workspace::Workspace}, state::WORKSPACES, win_api::{hook::set_event_hook, misc::get_main_module, window::{create_window, set_transparent}}};

pub fn init_hook() -> HWINEVENTHOOK {
  return set_event_hook(callback);
}

pub unsafe extern "system" fn callback (_: HWINEVENTHOOK, event: u32, hwnd: HWND, object_id: i32, child_id: i32, _: u32, _: u32) {
  match event {
